...
string ChatHelper::formatQuest(Quest const* quest, int32 loc)
{
	//将quest 显示为中文， 这里应该改为按照 bot->worldsession 里面的 dbclocales 来判断，因为其他地方也要改一改，添加个参数，给个默认值

	//string  的 = 重载 是 目标的字符串 cp 给 string，所以，这里要用string对象，而不能生成 string* ，string*会是nullptr
	string title;
	sObjectMgr.GetQuestLocaleStrings(quest->GetQuestId(), sObjectMgr.GetIndexForLocale(LocaleConstant(loc)), &title);
	if (title.empty()) {
		title.append(quest->GetTitle());
	}
...
string ChatHelper::formatSpell(SpellEntry const *sInfo, int32 loc)
{
	char* name = sInfo->SpellName[loc];
	if (!name || strlen(name) == 0) {
		name = sInfo->SpellName[DEFAULT_LOCALE];
	}
...
string ChatHelper::formatItem(ItemPrototype const * proto, int count, int32 loc)
{
	char color[32];
	...
	//添加中文查询
	string  name;
	sObjectMgr.GetItemLocaleStrings(proto->ItemId, sObjectMgr.GetIndexForLocale(LocaleConstant(loc)), &name);
	if (name.empty()) {
		name = proto->Name1;
	}
